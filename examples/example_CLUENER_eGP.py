...
from lit_ie.utils import pad_sequence, load_jsonl, print_average_token_length, truncate_data, \
...
class Module(LitModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        output['logits'] = output['logits'].to(torch.float32).numpy(force=True)
        ...
        for b, example in enumerate(batch['examples']):
            ...
            for label, start, end in zip(*np.where(output['logits'][b] > 0.)):
