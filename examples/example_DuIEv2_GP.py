...
def read_schema(file_name):
    ...
    for record in load_jsonl(file_name):
        for key in record['object_type']:
            schemas.append((record['subject_type'], record['predicate'], record['object_type'][key]))
            schema2key[record['subject_type'], record['predicate'], record['object_type'][key]] = key

    schema2id = {schema: i for i, schema in enumerate(schemas)}
    id2schema = {i: schema for i, schema in enumerate(schemas)}

    return schema2id, id2schema, schema2key


# 删掉文件中的第一行注释
schema2id, id2schema, schema2key = read_schema('data/DuIE2.0/duie_schema/duie_schema.json')
...
class Module(LitModule):
    ...
    def __init__(
        ...
        self.entities = GlobalPointer(config.hidden_size, head_hidden_size, 2, mask_tril=True, max_rope_len=max_length)
        self.subjects = GlobalPointer(
            config.hidden_size, head_hidden_size, len(schema2id), max_rope_len=max_length, use_rope=False
        )
        self.objects = GlobalPointer(
            config.hidden_size, head_hidden_size, len(schema2id), max_rope_len=max_length, use_rope=False
        )
    ...
    def on_test_epoch_end(self):
        ...
        id2example = collections.defaultdict(lambda: collections.defaultdict(list))
        for record in records:
            for relation in record['relations']:
                key = schema2key[relation['subject']['label'], relation['label'], relation['object']['label']]

                id2example[record['id']][
                    (relation['subject']['text'], relation['subject']['label'], relation['label'])
                ].append(
                    (key, relation['object']['text'], relation['object']['label'])
                )
                ...
                'spo_list': [
                    {
                        ...
                        'object': {key: text for key, text, _ in id2example[id][sbj, sbj_type, pred]},
                        'object_type': {key: value for key, _, value in id2example[id][sbj, sbj_type, pred]}
                    }
                    for sbj, sbj_type, pred in id2example[id]
                    if '@value' in {key for key, _, _ in id2example[id][sbj, sbj_type, pred]}
                ]}
