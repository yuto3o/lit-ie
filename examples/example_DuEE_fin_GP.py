...
def read_schema(file_name):
    ...
    for record in load_jsonl(file_name):
        ...
        for elem in record['role_list']:
            if 'enum_items' in elem:
                for enum_type in elem['enum_items']:
                    schemas.append((record['event_type'], enum_type))
                    schema2enum[(record['event_type'], enum_type)] = elem['role']

                enums[(record['event_type'], elem['role'])] = elem['enum_items']
            else:
    ...
    return schema2id, id2schema, enums, schema2enum


schema2id, id2schema, enums, schema2enum = read_schema('data/DuEE-fin/duee_fin_event_schema.json')
...
class DataModule(LitDataModule):
    ...
    def process_data(self, data):
        ...
        for record in load_jsonl(data):
            example = {'id': record['id'], 'text': normalize(record['title'] + '\n' + record['text']), 'events': []}

            if 'event_list' in record:

                for eve in record['event_list']:
                    tri_start, tri_end = search(normalize(eve['trigger']), example['text'])

                    if tri_start == -1 or tri_end == -1:
                        continue

                    event = {
                        'label': eve['event_type'],
                        'text': normalize(eve['trigger']),
                        'start': tri_start,
                        'end': tri_end,
                        'arguments': []
                    }
                    ...
                    for arg in eve['arguments']:
                        if (eve['event_type'], arg['role']) in enums:
                            ...
                            arg_start, arg_end = search(normalize(arg['argument']), example['text'])

                            if arg_start == -1 or arg_end == -1:
                                continue

                            event['arguments'].append(
                                {
                                    'label': arg['role'],
                                    'text': normalize(arg['argument']),
                                    'start': arg_start,
                                    'end': arg_end,
                                }
                            )
        ...
        for record in records:
            for event in record['events']:
                for arg in event['arguments']:
                    # recover enums
                    if (event['label'], arg['label']) in schema2enum:
                        text = arg['label']
                        label = schema2enum[(event['label'], arg['label'])]
