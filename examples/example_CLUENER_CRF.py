...
from lit_ie.utils import load_jsonl, print_average_token_length, truncate_data, print_summary_data, eval_metrics
from lit_ie.lit_module import LitModule, LitDataModule
from lit_ie.module import ConditionalRandomField
        ...
        parser.set_defaults(
            {
                ...
                'model_checkpoint.monitor': 'ent_macro_f1',
...
class DataModule(LitDataModule):
    ...
    def __init__(
            ...
            max_length: int = 2048,
            label_pad_token_id: int = schema2id['others'] * 2,
    ):
        super().__init__(
        ...
        for record in load_jsonl(data):
            ...
            if 'label' in record:
                ...
                for label in record['label']:
                    for text in record['label'][label]:
                        for start, end in record['label'][label][text]:
...
class Module(LitModule):
    ...
    def __init__(
        ...
        self.crf = ConditionalRandomField(len(schema2id) * 2 - 1)
    ...
    def forward(
        ...
        output = {
            'logits': self.crf(logits, attention_mask),
            'loss': None
        }

        if labels is not None:
            output['loss'] = self.crf.neg_log_likelihood_loss(logits, labels, attention_mask)
