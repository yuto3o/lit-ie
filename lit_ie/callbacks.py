...
from lightning.pytorch.utilities.types import TrainingStep


class RDropCallback(Strategy):
    """
    ...
    def training_step(self, *args, **kwargs):
        ...
        with self.precision_plugin.train_step_context():
            ...
            hf_inputs = kwargs.get('hf_inputs')
            attention_mask = hf_inputs['attention_mask']

            outputs0 = self.model(**hf_inputs)
            outputs1 = self.model(**hf_inputs)

            loss = 0.5 * (outputs0['loss'] + outputs1['loss'])
            ...
            for key in outputs0:
                if 'logits' in key:
                    kl_loss = (
                                      F.kl_div(
                                          F.log_softmax(outputs0[key], dim=-1),
                                          F.log_softmax(outputs1[key], dim=-1),
                                          reduction='none'
                                      ) +
                                      F.kl_div(
                                          F.log_softmax(outputs1[key], dim=-1),
                    ...
                    if outputs0[key].ndim == 2:
                        pad_mask = attention_mask[..., None]
                    elif outputs0[key].ndim == 3:
                        pad_mask = torch.einsum('bm,bn->bmn', attention_mask, attention_mask)[..., None]
                    else:
                        raise NotImplementedError()

                    kl_loss.masked_fill_(pad_mask, 0.)
                    kl_loss = kl_loss.sum() / (1 - pad_mask).sum()

                    kl_losses.append(kl_loss)

            return loss + self.alpha * torch.stack(kl_losses).mean()


class EMACallback(Callback):
    """
    Implements Exponential Moving Averaging (EMA).
    https://github.com/NVIDIA/NeMo/blob/67de251e980ef8d8aec3b8339e70e96f6c952122/nemo/collections/common/callbacks/ema.py#L27
    ...
    def on_fit_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        rank_zero_info("EMA callback start! Optimizers are replaced with EMAOptimizer.")
